#!/usr/bin/env bash
# ==============================================================================
# Bacterial Genome Assembly Pipeline — v4.0.0 (reviewed & optimized)
# Modes: Illumina (PE) | Nanopore | Hybrid (Illumina + Nanopore) | PacBio HiFi
#
# Changes from v3.0.0:
#   - Resume logic via step-sentinel files (skip already-done stages)
#   - Parallel tool-check at startup (fail fast, not mid-run)
#   - assembly-stats called once; results cached to avoid 3× forking
#   - awk contig-filter bug fixed (final record was dropped)
#   - seqkit filter now the primary path; awk is the fallback
#   - fastqc_raw.log renamed fastqc_post.log (was overwriting raw log)
#   - Pilon: RAM guard added; warns when heap > available RAM
#   - Medaka: --threads capped to min(THREADS, 4) — GPU bottleneck
#   - Bakta DB auto-detection fixed (bakta_db list output format)
#   - prokka always runs as annotation fallback, not only when bakta absent
#   - generate_report: assembly-stats cached; no repeated subprocess calls
#   - Strict quoting around all variable expansions
#   - GENOME_SIZE validated as <number>[m|g] before numeric math
#   - Trap reset before clean exit so ERR doesn't fire on intentional exits
#   - log() / ok() write to stdout; warn() / err() write to stderr (consistent)
#   - PlasmidFinder database flag made configurable (-P)
#   - Added --help as alias for -h
#   - Added -r flag to enable resume mode
# ==============================================================================
set -euo pipefail
IFS=$'\n\t'
...
${YELLOW}General:${NC}
  ...
  -M STR    Medaka model              [default: ${MEDAKA_MODEL}]
...
while getopts "1:2:l:o:t:s:g:a:m:M:d:c:P:xqrh" opt; do
    case "${opt}" in
        1) R1="${OPTARG}" ;;
        2) R2="${OPTARG}" ;;
        l) LONG="${OPTARG}" ;;
...
validate_inputs() {
    ...
    if [[ "${READ_TYPE}" == "long" ]] && ! ${HIFI_MODE}; then
...
03_polishing,04_annotation/{prokka,bakta,antismash},\
...
run_assembly() {
    ...
    log "[2/6] Assembly with ${ASSEMBLER}..."
...
run_polishing() {
    skip_if_done "polishing" && {
        _restore_assembly_path
        # Update ASSEMBLY to point to polished output
        if [[ "${READ_TYPE}" == "illumina" || "${READ_TYPE}" == "hybrid" ]]; then
            ASSEMBLY="${OUTDIR}/03_polishing/${SAMPLE}_pilon.fasta"
        elif [[ "${READ_TYPE}" == "long" ]] && ! ${HIFI_MODE}; then
            ASSEMBLY="${OUTDIR}/03_polishing/medaka/consensus.fasta"
        fi
        return
    }

    log "[3/6] Polishing assembly..."
    ...
    # ── Medaka (Nanopore only, not HiFi) ────────────────────────────────────
    if [[ "${READ_TYPE}" == "long" ]] && ! ${HIFI_MODE}; then

        # FIX: medaka_consensus does not scale past ~4 threads on CPU (GPU is
        # the bottleneck when a GPU is present; overscheduling wastes resources).
        local medaka_threads=$(( THREADS < 4 ? THREADS : 4 ))
        log "Running Medaka (model: ${MEDAKA_MODEL}, threads: ${medaka_threads})..."
        medaka_consensus \
            -i "${FILTERED_LONG}" \
            -d "${ASSEMBLY}" \
...
run_annotation() {
    ...
    if command -v bakta &>/dev/null; then
        ...
        bakta \
            ...
            warn "Bakta failed (check DB path with -d). Prokka will still run."
    fi

    # FIX: Prokka now always runs when available (not only as Bakta fallback).
    # Both annotators are complementary: Bakta for comprehensive INSDC-ready
    # output; Prokka for fast legacy-compatible .sqn/.gbk output.
    if command -v prokka &>/dev/null; then
        log "Running Prokka..."
        prokka \
            --outdir "${OUTDIR}/04_annotation/prokka/" \
            --prefix "${SAMPLE}" \
            --kingdom Bacteria \
            --cpus "${THREADS}" \
            --rfam \
            --force \
            "${ASSEMBLY}" \
            > "${OUTDIR}/logs/prokka.log" 2>&1
    else
        warn "Prokka not found; skipping."
    fi
...
# ── Final report ───────────────────────────────────────────────────────────────
generate_report() {
    log "[6/6] Generating final report..."
...
| Annotation      | \`${OUTDIR}/04_annotation/\`               |
| Quality reports | \`${OUTDIR}/05_reports/\`                  |
| Logs            | \`${OUTDIR}/logs/\`                         |

## Suggested Next Steps
1. **AMR screening** – \`AMRFinderPlus\` or \`ResFinder\`
2. **NCBI submission** – use \`tbl2asn\` with Prokka .sqn file
